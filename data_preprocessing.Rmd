...
Most of the created sf objects are of class 'POINT', but two objects, outdoor_fitness and tree_climbing are multipoints and multipolygons. 
```{r}
st_geometry(outdoor_fitness)
st_geometry(tree_climbing)
```

In order to get them all to be the same geometry, points, we divide the sf objects into two dataframes.

For some reason, the outdoor_fitness data is set as geometry type multipoint, even though it is only one point. This converts them into points
```{r}
df_points <- rbind(playgrounds, shelters, outdoor_fitness)
...
coords_poly <- st_coordinates(df_polygons)

df_polygons <- cbind(df_polygons, coords_poly)

```


# extract coordinates from points and polygons and bind them together to one dataframe 
```{r}
...
df_all <- rbind(df_points, df_polygons)
...
```{r}
write_csv(df_all_bounded, 'data/final_data.csv')
```


```{r}
testerrrrr <- read_csv('data/final_data.csv')
```
...
```{r}
testy <- read_csv('data/final_data.csv')

df_test <- st_cast(df_all)

st_write(df_test, 'data/test.shp')
```

```{r}
testerr <- st_centroid(df_polygons)
coords_test <- st_coordinates(testerr)

df_polygons <- cbind(df_polygons, coords_test)

df_polygons <- df_polygons %>% 
  rename(longitude = X,
         latitude = Y)

df_all_poly <- rbind(df_all, df_polygons)
```
...
c <- leaflet() %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addMarkers(lng=df_all$longitude, lat=df_all$latitude, popup = paste0('<b>', df_all$name,'</b>','<br/>', '<i>',  df_all$description, '</i>'), clusterOptions = markerClusterOptions()) %>%
  addPolygons(data = df_all[148:152, ], label = df_all$name)
c
```
