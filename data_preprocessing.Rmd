...
shelters <- create_sf(shelters_json, 'navn', 'Shelter', 'Outdoor shelter', 'Outdoor', 'Both children and adults')

tree_climbing <- create_sf(climbing_json, 'navn', 'Sport and fitness', 'Tree climbing', 'Outdoor', 'Both children and adults')

forests <- create_sf(forests_json, 'navn', 'Nature', 'Forest', 'Outdoor', 'Both children and adults')

firepits <- create_sf(firepits_json, 'navn', 'Nature', 'Bonfire pits in forest', 'Outdoor', 'Both children and adults')
```
...
c <- leaflet() %>%
  ...
  addMarkers(lng=df_all_bounded$longitude, lat=df_all_bounded$latitude, popup = paste0('<b>', df_all_bounded$name,'</b>','<br/>', '<i>',  df_all_bounded$description, '</i>'))
c  # Print the map
```
